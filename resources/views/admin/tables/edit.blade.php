                    ...
                    <div class="container mx-auto">
                        <form method="POST" action="{{ route('admin.tables.update', $table->id)}}" enctype="multipart/form-data">
                            ...
                            <div>
                                ...
                                <input type="text" name="name" class="w-full py-2 rounded" value="{{$table->name}}">
                                @error('name')
                                <span class="text-red-600">{{ $message }}</span>
                                @enderror
                            </div>
                            <div>
                                <label for="title">Gues Number</label>
                                <input type="number" name="guest_number" class="w-full py-2 rounded" value="{{$table->guest_number}}">
                                @error('guest_number')
                                <span class="text-red-600">{{ $message }}</span>
                                @enderror
                            </div>
